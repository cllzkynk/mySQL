...
-----------------------------------------------------------------------------*/ 
    ...
    -- group by ile where keywordu kullanilabilir.
    SELECT * FROM manav;
...
-----------------------------------------------------------------------------*/ 
 ...
 -- group by dan sonra having cümleciği kullanilir.
  ...
  eşit olmayan kayıtları listelemelidir.NOT: Sorgu, sadece MAKS urun_miktari MIN urun_miktarına 
  eşit olmayan kayıtları listelemelidir.
 -----------------------------------------------------------------------------*/   
 select urun_adi, max(urun_miktari) maks -- urun miktarına gore grupladik
 from manav
 group by urun_adi
 having maks!=min(urun_miktari) -- mx miktarı min miktarina esitse having kullandik eişt olanlari alma anlamında --> != yerine <> kullanilabilir
 order by maks;
 /*============================= DISTINCT =====================================
